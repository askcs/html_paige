...
ASKCache.prototype.array2list = function(arr){
	...
	function add(element) {
		...
		var id = element[cache.idQuery]; // nicer in its beauty?
...
ASKCache.prototype.sync =	function(sucCallback,failedCallback){
	...
	function applyChanges(){
		...
		var sucCB = (typeof sucCallback == "undefined")? function(){}: sucCallback;
		var failedDB = (typeof failedCallback == "undefined")? function(){}: failedCallback;
		function applyChange(func,item){
			cache.outstanding_request=true;
			func(cache.url+encodeURIComponent(item.id),cache.data,{
				url:cache.url+encodeURIComponent(item.id),
				data:item.data,
